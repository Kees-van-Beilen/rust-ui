#![warn(missing_docs)]
//! Run tasks on a separate thread and let them sync back periodically with the main thread.

use std::thread::{self, JoinHandle};

/// A object representing an async/multithreaded Task that is **not** executed on the main thread
pub struct Task {
    /// The internal join handle, You must'nt join a task on the main thread
...
impl Task {
    ...
    pub fn create<
        A: Send + 'static,
        Exe: FnOnce(&dyn Fn(A)) -> A + Send + 'static,
        Syn: Fn(A) + Clone + 'static,
    >(
        executor: Exe,
        sync: Syn,
    ) -> Self {
