...
impl RenderObject for crate::views::TextField {
    ...
    fn render(&self, mut data: crate::native::RenderData) -> Self::Output {
        ...
        let i = borrow
            .get_or_register_gc(self.identity.unwrap(), || {
                ...
                android_println!("habhbah1 {:?}", unsafe {
                    mem::transmute_copy::<_, *mut jni::sys::jobject>(text_view)
                });
        ...
        android_println!("habhbah3 {:?}", unsafe {
            mem::transmute_copy::<_, jni::sys::jobject>(i.deref())
        });
        // drop(borrow);
...
impl ComputableLayout for NativeTextView {
    fn preferred_size(
        ...
        android_println!("habhbah ---> {:?}", unsafe {
            mem::transmute_copy::<_, jni::sys::jobject>(&*self.0)
        });
