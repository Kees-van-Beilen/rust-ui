...
pub fn delegate_set_size<T: AsRef<android2_android::view::View<'static>> + Any>(
    this: &T,
    to: crate::layout::Size<f64>,
) {
    ...
    match env.set_field(&layout, "width", "I", (to.width as i32).into()) {
        Ok(_) => {}
        Err(e) => {
            crate::android_println!("Could not set layout width (child is floating) {e}")
        }
    }
    match env.set_field(&layout, "height", "I", (to.height as i32).into()) {
        Ok(_) => {}
        Err(e) => {
            crate::android_println!("Could not set layout height (child is floating)  {e}")
        }
    }
}

pub fn delegate_set_position(
    this: impl AsRef<android2_android::view::View<'static>>,
    to: crate::layout::Position<f64>,
) {
    let env = unsafe { get_env() };
