...
use crate::{
    ...
    native::{
        ActivityExtension, RenderData,
        android::{callback::CallbackBlockOnDismiss, class_loader::BottomSheetDialog},
        helper::get_env,
    },
    ...
    for SheetModalPresenterView<View, Sheet>
{
    ...
    fn render(&self, mut data: crate::native::RenderData) -> Self::Output {
        ...
        let (sheet, dialog) = bm.get_or_register_gc(*identity, || {
            ...
            let preferred = rendered_child.preferred_size(&Size {
                width: w as f64,
                height: std::f64::INFINITY,
            });
            let height = preferred.height.unwrap_or(100.0);
            rendered_child.set_size(Size {
                width: w as f64,
                height,
            });
            rendered_child.set_position(Position::default());

            let retained: crate::native::helper::Retained<BottomSheetDialog> =
                retain!(bottom_dialog, env);
            ((sheet, retained.clone()), move || {
