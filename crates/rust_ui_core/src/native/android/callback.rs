...
use jni::{NativeMethod, descriptors::Desc};

use crate::{
    android_println,
    native::{
        ACTIVITY, ENV,
        helper::{Retained, with_env},
    },
    retain,
};
...
pub fn get_constructor(
...
) -> Retained<android2_java::lang::reflect::Constructor<'static>> {
    let object = android2_java::lang::Object::new(env);
    let cls = object.get_class(env);
    let cls_loader = cls.get_class_loader(env);
    let bb = unsafe {
        env.new_direct_byte_buffer(CLASSES.as_ptr() as *mut u8, CLASSES.len())
            .unwrap()
    };
    let bb: android2_java::nio::ByteBuffer = unsafe { mem::transmute(bb) };
    let in_memory_class_loader =
        android2_dalvik::system::InMemoryDexClassLoader::new_2(&bb, &cls_loader, &mut env);
    let in_memory_class_loader: android2_dalvik::system::BaseDexClassLoader =
        in_memory_class_loader.into();
    ...
    let constructor = env
        .get_object_array_element(rust_test_constructors, 0)
        .unwrap();
    let constructor: android2_java::lang::reflect::Constructor =
        unsafe { mem::transmute(constructor) };
    // Desc::<jni::objects::JClass>::lookup(self, _)
    match env.register_native_methods(jclass, methods) {
        Ok(_) => {}
        Err(native_method) => {
            android_println!("errortje: {}", native_method.to_string())
        }
    }
...
pub fn init_callback_class<'jni, 'a: 'jni>(
    mut env: &mut jni::JNIEnv<'jni>,
) -> Retained<android2_java::lang::reflect::Constructor<'static>> {
    get_constructor(
        env,
        "RustUIOnClickCallback",
        &[jni::NativeMethod {
            name: "onClick".into(),
...
pub fn init_runnable_class<'jni, 'a: 'jni>(
    mut env: &mut jni::JNIEnv<'jni>,
) -> Retained<android2_java::lang::reflect::Constructor<'static>> {
    get_constructor(
        env,
...
pub fn init_on_dismiss<'jni, 'a: 'jni>(
    mut env: &mut jni::JNIEnv<'jni>,
) -> Retained<android2_java::lang::reflect::Constructor<'static>> {
    get_constructor(
        env,
...
pub fn init_text_watcher_class<'jni, 'a: 'jni>(
    mut env: &mut jni::JNIEnv<'jni>,
) -> Retained<android2_java::lang::reflect::Constructor<'static>> {
    get_constructor(
        env,
...
pub extern "system" fn text_change<'jni>(
    ...
    let callback: std::sync::MutexGuard<
        '_,
        Box<dyn Fn(jni::JNIEnv, jni::strings::JavaStr) + Send>,
    > = unsafe { env.get_rust_field(&instance, "rawFunctionBox").unwrap() };
...
pub extern "system" fn native_callback_on_dismiss<'jni>(
    ...
    let callback: std::sync::MutexGuard<'_, Box<dyn Fn(jni::JNIEnv) + Send>> =
        unsafe { env.get_rust_field(&instance, "rawFunctionBox").unwrap() };
...
impl<'a> AsRef<android2_android::view::view::OnClickListener<'a>> for CallbackBlock<'a> {
    fn as_ref(&self) -> &android2_android::view::view::OnClickListener<'a> {
        unsafe {
            mem::transmute::<&CallbackBlock, &android2_android::view::view::OnClickListener<'a>>(
                self,
            )
        }
    }
...
impl<'a> AsRef<android2_android::content::dialog_interface::OnDismissListener<'a>>
    for CallbackBlockOnDismiss<'a>
{
    fn as_ref(&self) -> &android2_android::content::dialog_interface::OnDismissListener<'a> {
        unsafe {
            mem::transmute::<
                &CallbackBlockOnDismiss,
                &android2_android::content::dialog_interface::OnDismissListener<'a>,
            >(self)
        }
    }
}
...
impl<'local> CallbackBlockOnDismiss<'local> {
    pub fn new<'jni: 'local>(
        env: &mut jni::JNIEnv<'jni>,
        callback: impl Fn(jni::JNIEnv) + 'static,
    ) -> Self {
        CALLBACK_ON_DISMISS_CONSTRUCTOR.with(|constructor| {
            ...
            unsafe {
                ...
                env.set_rust_field(&instance, "rawFunctionBox", fake_box)
                    .unwrap();
            };
            Self { inner: instance }
        })
...
impl<'local> RunnableBlock<'local> {
    pub fn new<'jni: 'local>(
        env: &mut jni::JNIEnv<'jni>,
        callback: impl FnOnce(jni::JNIEnv) + 'static,
    ) -> Self {
...
impl<'local> TextChangeListener<'local> {
    pub fn new<'jni: 'local>(
        env: &mut jni::JNIEnv<'jni>,
        callback: impl Fn(jni::JNIEnv, jni::strings::JavaStr) + 'static,
    ) -> Self {
        TEXT_WATCHER_CONSTRUCTOR.with(|constructor| {
            ...
            let instance = constructor.new_instance(&empty_array, env);
            unsafe {
                let boxed: Box<dyn Fn(jni::JNIEnv<'_>, jni::strings::JavaStr) + 'static> =
                    Box::new(callback);
                // SAFETY:
                // Android runs on a single thread, or at least I hope 🙏
                // There should be a better less error prone way of doing this.
