...
use crate::{
    ...
    native::{
        RenderData,
        ios::app::{RustViewController, create_window},
    },
    ...
    for SheetModalPresenterView<View, Sheet>
{
    ...
    fn render(&self, data: crate::native::RenderData) -> Self::Output {
        ...
        type PersistData<'a, Sheet: RenderObject + 'static> =
            Option<(RenderData<'a>, Cell<Sheet::Output>)>;
        ...
        if let Some((sheet_generator, identity)) = &self.sheet {
            ...
            let mtm = MainThreadMarker::new().unwrap();
            {
                ...
                if let Some(Some((render_data, previous_sheet))) =
                    borrow.get::<PersistData<Sheet>>(*identity)
                {
                    if show_sheet {
                        ...
                        data.persistent_storage
                            .borrow_mut()
                            .insert::<PersistData<Sheet>>(*identity, None);
                    }
                } else {
                    if show_sheet {
                        let sheet = sheet_generator();
                        ...
                        let controller: Retained<RustViewController> =
                            unsafe { msg_send!(RustViewController::alloc(mtm), init) };
                        ...
                        // });
                        unsafe {
                            root_controller.presentViewController_animated_completion(
                                &controller,
                                true,
                                None,
                            )
                        ...
                        let mut size: Size<f64> =
                            unsafe { root_controller.view().unwrap().frame().size.into() };
                        size.height -= 100.0;
                        ...
                        data.persistent_storage
                            .borrow_mut()
                            .insert::<PersistData<Sheet>>(
                                *identity,
                                Some((render_data, Cell::new(render))),
                            );
