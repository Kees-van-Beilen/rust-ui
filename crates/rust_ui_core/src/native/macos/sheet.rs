...
impl RustWindowDelegate {
    ...
    pub fn new_attached_to_window(sheet:Box<dyn ComputableLayout>,window:&Retained<NSWindow>)->Retained<RustWindowDelegate>{
...
type PersistData<'a> = Option<(Retained<NSWindow>, RenderData<'a>,Retained<RustWindowDelegate>)>;
        ...
        if let Some((sheet_generator, identity)) = &self.sheet {
            ...
            if let Some(Some((window, render_data,del))) = borrow.get::<PersistData>(*identity)
            {
                if show_sheet {
                    ...
                    let _ = del;
                    drop(borrow);
                    data.persistent_storage.borrow_mut().insert::<PersistData>(*identity, None);
                }
            }else{
                if show_sheet {
                    let sheet = sheet_generator();
