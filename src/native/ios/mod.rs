...
pub mod native {
    ...
    use crate::{
        icon::Icon, layout::{self, ComputableLayout, RenderObject}, view::{mutable::MutableViewRerender, resources::{Resource, ResourceStack}}, views::{tabbar::RenderedTabData, FontFamily, FontSize, FontWeight}
    };
    use block2::RcBlock;
    ...
    use objc2_ui_kit::{
        ...
        UIFontWeightThin, UIFontWeightUltraLight, UIImage, UILabel, UITab, UITabBar,
        UITabBarController, UITabBarItem, UIView, UIViewController,
    };
        ...
        fn render(&self, data: crate::native::RenderData) -> Self::Output {
            ...
            let view = unsafe {
                ...
                let callback: block2::RcBlock<dyn Fn(NonNull<UIAction>)> =
                    block2::RcBlock::new(move |action: NonNull<UIAction>| cb());
    ...
    impl RenderObject for crate::views::Text {
        ...
        fn render(&self, data: crate::native::RenderData) -> Self::Output {
            ...
            let view = unsafe {
                ...
                //             .as_ref()
                //             .unwrap_or(&crate::views::FontFamily::SystemUI);
                //         if let FontFamily::SystemUI = family {
                //             let size = font.size.unwrap_or(UIFont::systemFontSize());
                //             let weight = font.weight.unwrap_or(FontWeight::Regular);
                //             let font = UIFont::systemFontOfSize_weight(
                //                 size,
                //                 font_weight_to_ui_font_weight(weight),
                //             );
                //             view.setFont(Some(&font));
                //         }
                //     }
    ...
    // impl UIViewRepresentable for Text {
    //     fn ui_view(&self) -> &UIView {
    //         &self.0
    //     }
    // }
    // impl UIViewRepresentable for Button {
    //     fn ui_view(&self) -> &UIView {
    //         &self.0
    //     }
    // }
    impl ComputableLayout for Text {
        fn preferred_size(&self, in_frame: &layout::Size<f64>) -> Option<layout::Size<f64>> {
    ...
    impl ComputableLayout for Button {
        fn preferred_size(&self, in_frame: &layout::Size<f64>) -> Option<layout::Size<f64>> {
            let mut frame = unsafe { self.0.sizeThatFits((*in_frame).into()) };
            Some(frame.into())
        }
        fn set_size(&mut self, to: crate::layout::Size<f64>) {
            let view = &self.0;
            ...
            let views = unsafe {
                ...
                let views = self.children.for_each_render(|data: RenderedTabData| {
                    ...
                    let itm = UITab::initWithTitle_image_identifier_viewControllerProvider(
                        ...
                        Some(&*RcBlock::new(move |tab: NonNull<UITab>| {
